...
/*Pushes elements from stack_a to stack_b that is within the range (min_val, max_val).
**For each element in stack_a, push in stack_b if its value is in the chunk range,
...
static void	push_chunk_to_b(node_t **stack_a, node_t **stack_b, int min_val, int max_val)
{
	int size;
	int i;
...
/*Iterates over all chunks and pushes the corresponding elements from stack_a to stack_b using push_chunk_to_b.
...
static void push_chunks(node_t **stack_a, node_t **stack_b, int min_val, int max_val, int chunks)
{
	int i = 0;

	while (i < chunks && *stack_a)
	{
		int chunk_min = min_val + (max_val - min_val) * i / chunks;
		int chunk_max = min_val + (max_val - min_val) * (i + 1) / chunks;
...
void	push_max_to_a(node_t **stack_a, node_t **stack_b)
{
	int max_val;
	int max_pos;
	int size;
...
void chunk_sort(node_t **stack_a, node_t **stack_b, int size)
{
	...
	push_chunks(stack_a, stack_b, min_val, max_val, chunks);
